// Keyword search / plain catalog list for OperationsPage, used when the
// search box's text is two words or fewer (and no ?intent= is set):
      ...
      {matchedOn && matchedOn.length > 0 && (
        <span className="hidden shrink-0 gap-1 sm:flex">
          {matchedOn.slice(0, 3).map((m) => (
            <span key={m} className="rounded bg-slate-100 px-1.5 py-0.5 text-[11px] text-slate-500 dark:bg-slate-800 dark:text-slate-400">
              {m}
            </span>
          ))}
        </span>
